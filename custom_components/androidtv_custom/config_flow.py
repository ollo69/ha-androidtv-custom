...
class OptionsFlowHandler(OptionsFlow):
    ...
    @callback
    def _async_commands_form(self, cmd_id: str | None) -> FlowResult:
        ...
        data_schema = vol.Schema(
            {
                vol.Optional(
                    CONF_CMD_VALUE,
                    description={
                        "suggested_value": self._cust_commands.get(cmd_id, "")
                    },
                ): TextSelector(config=TextSelectorConfig(multiline=True)),
            }
        )
